...
function parseContentDispositionAttribute(input, position) {
  ...
  const attributeName = collectASequenceOfBytes(char => isToken(char) && char !== 0x3d && char !== 0x2a,
  // not = or *
  input, position);
  ...
  if (isExtended) {
    ...
    const headerValue = collectASequenceOfBytes(char => char !== 0x20 && char !== 0x0d && char !== 0x0a && char !== 0x3b,
    // not space, CRLF, or ;
    input, position);

    // Check for utf-8'' prefix (case insensitive)
    if (headerValue[0] !== 0x75 && headerValue[0] !== 0x55 ||
    // u or U
    headerValue[1] !== 0x74 && headerValue[1] !== 0x54 ||
    // t or T
    headerValue[2] !== 0x66 && headerValue[2] !== 0x46 ||
    // f or F
    headerValue[3] !== 0x2d ||
    // -
    ...
    const quotedValue = collectASequenceOfBytes(char => char !== 0x0a && char !== 0x0d && char !== 0x22,
    // not LF, CR, or "
    input, position);
    ...
    const tokenValue = collectASequenceOfBytes(char => isToken(char) && char !== 0x3b,
    // not ;
    input, position);
...
const _multipartFormDataParser = _cjs_default["multipartFormDataParser"];
export { _multipartFormDataParser as multipartFormDataParser };
const _validateBoundary = _cjs_default["validateBoundary"];
export { _validateBoundary as validateBoundary };
